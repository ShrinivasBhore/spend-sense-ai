...
export const ExpenseList: React.FC = () => {
  ...
  const filteredAndSorted = useMemo(() => {
    ...
    return result;
  }, [currentMonthExpenses, searchTerm, filterCategory, sortBy]);
  ...
  if (currentMonthExpenses.length === 0) {
    return (
      <div className="bg-white p-8 rounded-2xl shadow-sm border border-slate-100 text-center">
        ...
        <h3 className="text-lg font-medium text-slate-800 mb-2">No expenses this month</h3>
  ...
  return (
    <div className="bg-white rounded-2xl shadow-sm border border-slate-100 overflow-hidden">
      <div className="p-6 border-b border-slate-100 space-y-4">
        <h3 className="text-lg font-semibold text-slate-800">Monthly Expenses</h3>
      ...
      <div className="divide-y divide-slate-100 max-h-[600px] overflow-y-auto">
        {filteredAndSorted.length === 0 ? (
          <div className="p-8 text-center text-slate-500">No expenses match your filters.</div>
        ) : (
                  ...
                  <div className="flex items-start gap-4">
                    ...
                    <div>
                      ...
                      <div className="flex items-center gap-3 mt-1 text-sm text-slate-500 flex-wrap">
                        <span className="flex items-center gap-1"><Tag className="w-3.5 h-3.5" />{expense.category}</span>
                        <span className="flex items-center gap-1"><CreditCard className="w-3.5 h-3.5" />{expense.paymentMethod}</span>
                        <span className="flex items-center gap-1"><Calendar className="w-3.5 h-3.5" />{format(parseISO(expense.date), 'MMM d, yyyy')}</span>
                      </div>
                    </div>
                  </div>
